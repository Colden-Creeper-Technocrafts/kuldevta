...
@section('title', __('sangh.registrations'))
@section('page-title', $sangh->title() . ' — ' . __('sangh.registrations'))
...
<div class="row g-4">
    ...
    <div class="col-lg-4">
        <div class="card border-success">
            ...
            <div class="card-body">
                <p class="text-muted small">Enter mobile number to confirm devotee is present and walking.</p>
                <form method="POST" action="{{ route('admin.sangh.registrations.confirm', $sangh) }}">
                    @csrf
        ...
        <div class="card mt-3">
            <div class="card-header"><strong><i class="bi bi-person-plus me-1"></i>Add Registration</strong></div>
            <div class="card-body">
                <form method="POST" action="{{ route('admin.sangh.registrations.store', $sangh) }}">
                    @csrf
                    <div class="mb-2">
    ...
    <div class="col-lg-8">
        <div class="card">
            <div class="card-header">
                <form method="GET" action="{{ route('admin.sangh.registrations', $sangh) }}" class="d-flex gap-2">
            ...
            <div class="card-body p-0">
                @if($registrations->isEmpty())
                    <p class="text-center text-muted py-4">No registrations found.</p>
                @else
                <div class="table-responsive">
                    <table class="table table-hover table-sm mb-0">
                        ...
                        <tbody>
                            ...
                            <tr @if($reg->group_leader_id) style="background:#fafafa" @endif>
                                <td class="text-muted small">{{ $loop->iteration }}</td>
                                <td>
                                    @if($reg->group_leader_id)
                                    ...
                                    <div class="d-flex gap-1 mt-1">
                                        ...
                                        @endif
                                        @if($reg->age)
                                            <span class="badge bg-light text-muted border" style="font-size:.65rem">{{ $reg->age }}y</span>
                                        @endif
                                    </div>
                                </td>
                                <td>{{ $reg->mobile }}</td>
                                <td><code>{{ $reg->token }}</code></td>
                                <td class="text-muted small">{{ $reg->village ?? $reg->city ?? '—' }}</td>
                                <td>
                                    <span class="badge {{ $reg->statusBadgeClass() }}">{{ __('app.' . $reg->status) }}</span>
                                </td>
                                <td>
                                    <form method="POST" action="{{ route('admin.sangh.registrations.status', [$sangh, $reg]) }}">
                                        @csrf @method('PATCH')
                                        <select name="status" class="form-select form-select-sm" style="min-width:100px"
                                            ...
                                            @foreach(['registered','confirmed','completed','dropped'] as $s)
                                                <option value="{{ $s }}" {{ $reg->status == $s ? 'selected' : '' }}>{{ __('app.' . $s) }}</option>
                                            @endforeach
                                        </select>
                                    </form>
                                </td>
